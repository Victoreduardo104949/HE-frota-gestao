...
export default function App() {
  ...
  const handleAddTrip = (newTrip: Trip) => {
    ...
    // Sync driver status if trip is in-progress
    if (newTrip.status === 'in-progress') {
      setDrivers(prev => prev.map(d => 
        d.id === newTrip.driverId ? { ...d, status: 'on-trip' } : d
      ));
    }
  };
  ...
  const handleUpdateTrip = (updatedTrip: Trip) => {
    setTrips(prev => {
      const oldTrip = prev.find(t => t.id === updatedTrip.id);
      const newTrips = prev.map(t => t.id === updatedTrip.id ? updatedTrip : t);

      // Status transitioned to in-progress
      if (oldTrip?.status !== 'in-progress' && updatedTrip.status === 'in-progress') {
        setDrivers(prevDrivers => prevDrivers.map(d => 
          d.id === updatedTrip.driverId ? { ...d, status: 'on-trip' } : d
        ));
      }
      
      // Status transitioned FROM in-progress TO something else (completed/scheduled)
      if (oldTrip?.status === 'in-progress' && updatedTrip.status !== 'in-progress') {
        setDrivers(prevDrivers => prevDrivers.map(d => 
          d.id === updatedTrip.driverId ? { ...d, status: 'active' } : d
        ));
      }
