...
import { Card } from './UI';
import { MapPin, ArrowRight, Package, DollarSign, TrendingUp, Filter, Plus, Search, Truck, X, Calendar } from 'lucide-react';
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <button 
                        ...
                        className="text-elegant-accent hover:text-elegant-accent/80 transition-colors"
                      >
                    ...
                    <div className="space-y-1.5">
                      ...
                      {vehicles.some(v => v.status === 'maintenance') && (
                        <p className="text-[9px] text-elegant-danger/60 italic mt-1">
                          * Veículos em manutenção estão visíveis mas indisponíveis para seleção.
                        </p>
                      )}
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <select name="driverId" className="w-full bg-elegant-bg border border-elegant-border rounded px-3 py-2 text-sm text-elegant-text focus:outline-none focus:border-elegant-accent" required>
                        ...
                        {drivers.map(d => {
                          const isUnavailable = d.status !== 'active';
                          const statusLabel = 
                            d.status === 'vacation' ? '(EM FÉRIAS)' :
                            d.status === 'suspended' ? '(SUSPENSO)' :
                            d.status === 'on-trip' ? '(EM VIAGEM)' : '';
                      ...
                      {drivers.some(d => d.status !== 'active') && (
                        <p className="text-[9px] text-elegant-danger/60 italic mt-1">
                          * Motoristas ausentes ou em viagem estão visíveis mas indisponíveis.
                        </p>
                      )}
                    </div>
                    <div className="grid grid-cols-2 gap-4">
