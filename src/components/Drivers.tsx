    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredDrivers.map((driver) => {
          ...
          const activeTrip = trips.find(t => t.driverId === driver.id && t.status === 'in-progress');
          const scheduledTrip = trips.find(t => t.driverId === driver.id && t.status === 'scheduled');
          
          const isOnTrip = !!activeTrip;
          const isScheduled = !!scheduledTrip && !isOnTrip;
          
          const displayStatus = isOnTrip ? 'on-trip' : isScheduled ? 'scheduled' : driver.status;
            ...
            <Card key={driver.id} className="relative group">
              ...
              <div className="flex gap-2">
                <button 
                  ...
                  className="p-2 rounded bg-elegant-bg border border-elegant-border text-elegant-dim hover:text-elegant-accent hover:border-elegant-accent/50 hover:bg-elegant-accent/5 transition-all shadow-sm"
                  title="Editar Motorista / Mudar Status"
                >
                  <Edit2 size={14} />
                </button>
                <button 
                  onClick={() => handleOpenDeleteModal(driver)}
                  className="p-2 rounded bg-elegant-bg border border-elegant-border text-elegant-dim hover:text-elegant-danger hover:border-elegant-danger/50 hover:bg-elegant-danger/5 transition-all shadow-sm"
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center text-xs">
                ...
                <span className={cn(
                  ...
                  displayStatus === 'on-trip' ? "bg-elegant-accent/10 text-elegant-accent border-elegant-accent/20 shadow-[0_0_10px_rgba(59,130,246,0.1)]" :
                  displayStatus === 'scheduled' ? "bg-elegant-warning/10 text-elegant-warning border-elegant-warning/20 shadow-[0_0_10px_rgba(234,179,8,0.05)]" :
                  displayStatus === 'active' ? "bg-elegant-success/10 text-elegant-success border-elegant-success/20" :
                  displayStatus === 'vacation' ? "bg-elegant-warning/10 text-elegant-warning border-elegant-warning/20" :
                  "bg-elegant-danger/10 text-elegant-danger border-elegant-danger/20"
                )}>
                  {displayStatus === 'active' ? 'Disponível' : 
                   displayStatus === 'on-trip' ? 'Em Viagem' : 
                   displayStatus === 'scheduled' ? 'Agendado' :
                   displayStatus === 'vacation' ? 'Férias' : 'Suspenso'}
                </span>
              </div>
              <div className="flex justify-between items-center text-xs">
