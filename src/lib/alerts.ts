  ...
  vehicles.forEach(vehicle => {
    ...
    if (kmToMaintenance <= 0) {
      alerts.push({
        ...
        message: `${vehicle.name} (${vehicle.plate}) ultrapassou o limite de revisão em ${Math.abs(kmToMaintenance)} km.`,
        targetId: vehicle.id,
        targetType: 'vehicle',
      ...
      alerts.push({
        ...
        message: `${vehicle.name} está a ${kmToMaintenance} km da próxima revisão.`,
        targetId: vehicle.id,
        targetType: 'vehicle',
        date: now.toISOString(),
      });
    ...
    if (insuranceDate < now) {
      alerts.push({
        ...
        message: `O seguro do veículo ${vehicle.name} venceu em ${insuranceDate.toLocaleDateString('pt-BR')}.`,
        targetId: vehicle.id,
        targetType: 'vehicle',
        date: now.toISOString(),
      });
      ...
      alerts.push({
        ...
        message: `O seguro do veículo ${vehicle.name} vence em ${insuranceDate.toLocaleDateString('pt-BR')}.`,
        targetId: vehicle.id,
        targetType: 'vehicle',
        date: now.toISOString(),
      });
    ...
    if (vehicle.status === 'maintenance') {
      alerts.push({
        ...
        message: `${vehicle.name} está atualmente em manutenção.`,
        targetId: vehicle.id,
        targetType: 'vehicle',
        date: now.toISOString(),
      });
